...
const registrarAccionAuditoria = async (req, accion, detalles, severidad) => {
  try {
    // Evitamos colapsos si el objeto req completo llega nulo o vacío por contingencias
    if (!req) return;
    ...
    // 🌐 🛡️ CORRECCIÓN CRÍTICA: Encadenamiento opcional para evitar lecturas nulas sobre headers o sockets ausentes
    ...
    if (rawIp === '::1' || rawIp === '::ffff:127.0.0.1') {
      rawIp = '127.0.0.1';
    } else if (typeof rawIp === 'string' && rawIp.startsWith('::ffff:')) {
